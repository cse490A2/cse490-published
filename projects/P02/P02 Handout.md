...
The number of Large Language Models has exploded. From [Hugging Face](https://huggingface.co/models) alone, you can download over 3 million models, granted your hardware supports it\! With the ever-increasing rise of models, how do you tell which ones are good? Do you download the biggest LLM that your hardware supports? AI experts tackle this question through a plethora of methodologies, but the simplest approach is often the most effective: compare multiple models against a single prompt spec\! In this project, you will test a prompt spec on a range of models. But there's a catch: the spec must recreate an interactive arcade game in HTML format.

Everyone starts with the same spec: Breakout doubles. It is the arcade game with a twist no model has seen before: your paddle on the left half, a computer teammate's paddle on the right half that chases the ball on its own but only so fast, so it can miss. You write the prompt once, send it to each model, save each reply as a game file, and play it. Some will work first try. Most of the failures will be quiet: a teammate that never moves, or one that cheats. You fix one by changing the ask, not the code. Then, if you want to make it awesome, you write a spec of your own: a blend of two arcade games.
...
2. Write your prompt in PROMPT.md: start from the Breakout doubles prompt in the Reference section and put your theme into it: the colours, the teammate's name, and the words on the end screens. Keep the teammate's 3-pixel speed cap and the centre line; that is where the game breaks. If you are stuck, talk to a model on LiteLLM\!  
